...
- `howells-ratchet` resolves each unit's Oxfmt and Oxlint config from that unit's own directory. `measureFormatting` resolved the Oxfmt config from the process working directory instead, so in a monorepo every package was measured against whatever config sat at the repo root - or, in a repo with no root config, against the packaged preset - and a pinned `--config` applies to the whole run and beats the nested config the package's own lint would have discovered. Reported from the rollout against materialsinuse, where 9 of its 10 units failed `--migrate` on 874 files that `howells-check` passes inside those packages: each package config there is `{ extends: [howells] }`, and `packages/foundation` adds an `ignore` list for two drizzle-generated files, none of which the pinned preset carries.
- The error runs in both directions, so neither "phantom failures" nor "a false pass" describes it on its own: whichever of the two configs is stricter wins, and it is the wrong one either way. Measured both - materialsinuse fails on files its packages format correctly, while a fixture whose root sets a wider `printWidth` than the package passes a file the package rejects. The contract is now the one the gate is meant to enforce: each unit measures exactly what `howells-check` run inside that package measures, confirmed against materialsinuse's `packages/mastra` with that unit's own target list.
- Covered by three tests, each confirmed to fail with the guard broken: an end-to-end Oxfmt pair (a package whose config sets a narrower width fails on a file the root's width allows, and the same file passes once that config is removed), a module-level pair on `measureFormatting` contributed by ratchet-d, who found the bug, and an Oxlint-side test pinning the same property for finding counts. The Oxlint path already passed the unit's directory and needed no change; its test uses `no-console` against `eqeqeq` rather than `no-debugger`, which Oxlint enables by default and so cannot tell two configs apart.
