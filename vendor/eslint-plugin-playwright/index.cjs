/*
 * Vendored from eslint-plugin-playwright 2.11.0 (MIT, see LICENSE beside
 * this file). Oxlint runs it through its JS-plugin bridge. The published
 * package declares ESLint as a required peer, which pnpm installs in every
 * consumer; this copy needs only `globals`, so nothing installs ESLint.
...
var isSupportedAccessor = (node, value) =>
...
var resolveToPlaywrightFn = (context, accessor) => {
  ...
  return {
    original:
      resolvePossibleAliasedGlobal(context, resolved) ??
      resolveImportAlias(context, accessor),
    local: resolved,
  };
};
...
function parse(context, node) {
  ...
  if (
    chain.nodes
      ...
      .some((n) => n.parent?.type !== "MemberExpression")
  ) {
    return null;
  }
...
function isLiteral(node, type, value) {
  return (
    node.type === "Literal" &&
    (value === void 0 ? typeof node.value === type : node.value === value)
  );
}
var isTemplateLiteral = (node, value) =>
  node.type === "TemplateLiteral" &&
  ...
  if (obj.type === "MemberExpression") {
    const pattern =
      obj.object.type === "ThisExpression"
        ? pageFrameFullPattern
        : pageFramePrefixPattern;
...
var equalityMatchers = /* @__PURE__ */ new Set([
  "toBe",
  "toEqual",
  "toStrictEqual",
]);
...
var areTokensOnSameLine = (left, right) =>
  left.loc.end.line === right.loc.start.line;
var isPromiseAccessor = (node) => {
  return (
    node.type === "MemberExpression" &&
    isIdentifier(node.property, /^(then|catch|finally)$/)
  );
};
    ...
    return {
      CallExpression(node) {
        ...
        const isValid = result
          ? checkValidity(node, /* @__PURE__ */ new Set())
          : false;
        if (result && !isValid) {
          context.report({
            data: result.data,
    ...
    return {
      CallExpression(node) {
        ...
        if (call.group === "test" && call.type === "config") {
          const isSlowCall = call.members.some(
            (s) => getStringValue(s) === "slow"
          );
          if (isSlowCall) {
      ...
      CallExpression(node) {
        ...
        if (
          ...
          (node.arguments.length !== 0 ||
            findParent(node, "BlockStatement")?.parent?.type ===
              "IfStatement" ||
            findParent(node, "SwitchCase") !== void 0)
        ) {
          return;
        }
  ...
  create(context) {
    ...
    return {
      ...
      CallExpression(node) {
        ...
        if (call?.type === "expect") {
          if (
            call.head.node.parent?.type === "MemberExpression" &&
...
var locatorReturning = /* @__PURE__ */ new Set([
  ...locatorMethods2,
  "and",
  "filter",
  "first",
  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        if (
          modifierNames.some(
            (name) => name === "resolves" || name === "rejects"
          )
        ) {
          return;
        }
            ...
            ? [
                {
                  fix(fixer) {
                    ...
                    const alreadyAwaited =
                      matcherCall.parent?.type === "AwaitExpression";
                    if (!alreadyAwaited) {
                      fixes.unshift(
                        fixer.insertTextBefore(expectCall, "await ")
                      );
                    }
                    return fixes;
                  },
...
function isSupportedMethod(node) {
  ...
  return (
    locatorMethods3.has(name) ||
    (pageMethods2.has(name) && isPageMethod(node, name))
  );
}
...
var isFixableIncludesCallExpression = (node) =>
  node.type === "CallExpression" &&
  node.callee.type === "MemberExpression" &&
  ...
  create(context) {
    ...
    return {
      CallExpression(node) {
        ...
        if (
          tagValue.type === "Literal" ||
          tagValue.type === "TemplateLiteral"
        ) {
          if (
            tagValue.type === "Literal" &&
            typeof tagValue.value !== "string"
          ) {
            context.report({
...
var compileMatcherPatterns = (matchers2) => {
  ...
  return {
    describe: matchers2.describe
      ? compileMatcherPattern(matchers2.describe)
      : null,
...
var plugin = {
  ...
  rules: {
    "consistent-spacing-between-blocks":
      consistent_spacing_between_blocks_default,
